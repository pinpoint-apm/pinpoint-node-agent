...
class GrpcDataSender {
  ...
  sendSpanChunk(spanChunk) {
    try {
      ...
      const spanStream = new GrpcStream("spanChunk", () => {
        return this.spanClient.sendSpan((err, response) => {
          if (err) {
            log.debug(`GrpcStream spanChunkStream callback err: ${err}`)
          }
        })
      })
      spanStream.write(pSpanChunk)
      spanStream.end()
    } catch (e) {
      if (e && e.stack) {
  ...
  sendStat(stat) {
    try {
      ...
      const statStream = new GrpcStream("statStream", () => {
