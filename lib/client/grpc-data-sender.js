...
class GrpcDataSender {
  constructor(
    ...
    this.initializeClients(agentInfo, collectorIp, collectorTcpPort, collectorStatPort, collectorSpanPort)
  }
  ...
  initializeSpanStream(collectorIp, collectorSpanPort, headerInterceptor) {
    ...
    this.spanStream.on('data', function (data) {
      log.debug(`spanStream on(data) : ${JSON.stringify(data)}`)
    })
