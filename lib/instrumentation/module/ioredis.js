...
module.exports = function (agent, version, ioredis) {
  ...
  InstrumentMethod.make(ioredis.prototype, 'sendCommand').addScopedInterceptor(new IoredisSendCommandInterceptor())  
  ...
  function wrapSendCommand (original) {
    return function wrappedSendCommand(command) {
      ...
      if (command && trace) {
        ...
        asyncTrace = TraceBuilder.valueOfTrace(trace).buildAsyncTrace(spanEventRecorder.getLocalAsyncId())
