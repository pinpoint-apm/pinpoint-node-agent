...
class GrpcDataSender {
  ...
  initializeSpanStream(collectorIp, collectorSpanPort, headerInterceptor) {
    ...
    this.spanStream = new GrpcClientSideStream("spanStream", () => {
      return this.spanClient.sendSpan((err, response) => {
        if (err) {
          log.debug(`GrpcClientSideStream spanStream callback err: ${err}`)
        }
      })
    })
  }
  ...
  initializeStatStream(collectorIp, collectorStatPort, headerInterceptor) {
    ...
    this.statStream = new GrpcClientSideStream("statStream", () => {
      return this.statClient.sendAgentStat((err, response) => {
        if (err) {
          log.debug(`GrpcClientSideStream statStream callback err: ${err}`)
        }
      })
    })
  }
  ...
  initializePingStream() {
    this.pingStream = new GrpcBidirectionalStream('pingStream', () => {
