...
module.exports = function (agent, version, express) {
  ...
  function doPatchLayer(layer, objectName, layerName) {
    ...
    // const handlerBuilder = makeMethodDescriptorBuilder(MODULE_NAME, 4)
    ...
    shimmer.wrap(layer, 'handle', function (original) {
      return (original.length === 4)
        ...
        : recordHandle(original, objectName, layerName, appAllMethodDescriptor, layer.route.path)
    })
  }

  function recordHandle(original, objectName, layerName, appAllMethodDescriptor, path) {
    return function (req) {
