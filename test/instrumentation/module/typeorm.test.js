...
test('TypeORM default port handling', async (t) => {
    // Simplified test - just verify default port logic
    const TypeORMRecorder = require('../../../lib/instrumentation/module/typeorm/typeorm-recorder')
    
    const mysqlPort = TypeORMRecorder.getDefaultPort('mysql')
    const pgPort = TypeORMRecorder.getDefaultPort('postgres')
    const unknownPort = TypeORMRecorder.getDefaultPort('unknown')
    
    t.equal(mysqlPort, 3306, 'MySQL default port should be 3306')
    t.equal(pgPort, 5432, 'PostgreSQL default port should be 5432')
    t.equal(unknownPort, 0, 'Unknown database type should return 0')
