...
class SpanChunk {
  ...
  get spanMessage() {
    ...
    const localAsyncId = convertLocalAsyncId(this.localAsyncId)
    pSpanChunk.setLocalasyncid(localAsyncId)

    const keyTime = this.keyTime
    pSpanChunk.setKeytime(keyTime)
    
    const spanEvents = new SpanEvents(this.spanEventList)
    const pSpanEvents = localAsyncId ? spanEvents.getpAsyncSpanEvents(keyTime) : spanEvents.getpSpanEvents(keyTime)
