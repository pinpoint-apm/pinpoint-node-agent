...
class Agent {
  ...
  /**
   * When compiled with Webpack, the execution order can be changed, so HTTP hooking is not possible.
   * You can run it with `node -r` from https://github.com/pinpoint-apm/pinpoint-node-agent/issues/73. 
   * Otherwise, you need to call the start method before createServer so that the order of execution in webpack is not changed.
   */
  start() {
