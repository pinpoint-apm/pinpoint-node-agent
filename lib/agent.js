...
class Agent {
  constructor (initOptions) {
    ...
    const agentInfo = this.createAgentInfo(this.config, agentStartTime)

    this.initializePinpointClient(agentInfo);

    this.isSampling = sampler.getIsSampling(this.config.sampling, this.config.sampleRate)
    this.traceContext = traceContext.init(agentInfo, this.pinpointClient.dataSender)

    stringMetaService.init(agentInfo, this.pinpointClient.dataSender)
    apiMetaService.init(agentInfo, this.pinpointClient.dataSender)
  ...
  initializePinpointClient(agentInfo) {
    this.pinpointClient = new PinpointClient(this.config, agentInfo);
  ...
  spanEndCallbackWrapper(original, trace, spanEventRecorder) {
