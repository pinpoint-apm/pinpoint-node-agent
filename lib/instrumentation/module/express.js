...
module.exports = function (agent, version, express) {
  ...
  function recordHandle(original, moduleName, methodName) {
    return function (req, res, next) {
      ...
      if (trace) {
        let builder = new SpanEventRecorder.builder()
          .setServiceType(ServiceTypeCode.express)

        const methodDescriptor = getMethodDescriptor(moduleName, methodName, req.method)
        if (methodDescriptor) {
          builder = builder.setApi(getMethodDescriptor(moduleName, methodName, req.method))
        } else {
          builder = builder.setApiDesc(getDescriptionText(moduleName, methodName, req.method))
        }
        spanEventRecorder = trace.startSpanEvent(builder)
      }
      const result = original.apply(this, arguments)
      if (trace) {
