...
class TraceContext {
  constructor(agentInfo, dataSender, config, traceCompletionHooks = []) {
    ...
    this.traceCompletionHooks = traceCompletionHooks
  }
  ...
  completeTraceObject(trace) {
    ...
    try {
      trace.close()
      for (const hook of this.traceCompletionHooks) {
        try {
          hook.onComplete(trace)
        } catch (e) {
          this.logger.warn('Hook failed', e)
        }
      }
