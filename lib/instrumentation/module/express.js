...
const CallSite = require('../call-site')
  ...
  shimmer.wrap(express.Router, 'route', function (original) {
    return function () {
      ...
      if (this.stack && this.stack.length) {
        ...
        const httpMethodCallSite = makeHttpMethodCallSite(stacks)

        const layer = this.stack[this.stack.length - 1]
        doPatchLayer(layer, 'route', layer.name, httpMethodCallSite)
      }
      return result
    }
  })

  function doPatchLayer(layer, moduleName, methodName, httpMethodCallSite) {
    shimmer.wrap(layer, 'handle', function (original) {
      return (original.length === 4)
        ? recordErrorHandle(original, moduleName, methodName)
        : recordHandle(original, moduleName, methodName, httpMethodCallSite)
    })
  }

  function recordHandle(original, moduleName, methodName, httpMethodCallSite) {
...
function makeHttpMethodCallSite(stacks) {
