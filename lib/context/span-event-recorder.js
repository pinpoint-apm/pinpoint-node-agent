...
class SpanEventRecorder {
  ...
  record(builder) {
    if (builder.serviceType) {
      this.recordServiceType(builder.serviceType)
    }

    if (builder.apiDesc) {
      this.recordApiDesc(builder.apiDesc)
    }

    if (builder.destinationId) {
      this.recordDestinationId(builder.destinationId)
    }

    if (builder.endPointIP && builder.endPointPort) {
      this.recordEndPoint(addressStringOf(builder.endPointIP, builder.endPointPort))
    }

    if (builder.api) {
      this.recordApi(builder.api)
    }
  }
  ...
  static get builder() {
    ...
    return SpanEventRecorderBuilder
  }
}
