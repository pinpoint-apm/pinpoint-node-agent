...
class SpanEvents {
    ...
    getpAsyncSpanEvents(keyTime) {
        let events = []
        if (this.spanEvents.length > 0) {
            const firstSpanEvent = this.spanEvents[0]

            const asyncEvent = new SpanEvent({spanId: firstSpanEvent.spanId}, 0)
            asyncEvent.depth = 1
            asyncEvent.elapsedTime = firstSpanEvent.elapsedTime
            asyncEvent.serviceType = ServiceTypeCode.async
            asyncEvent.startTime = keyTime

            events.push(this.convertSpanEvent(asyncEvent, keyTime))
        }
        return this.pSpanEvents(events, keyTime)
    }
