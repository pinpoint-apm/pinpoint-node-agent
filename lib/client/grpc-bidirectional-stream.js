/**
 * Pinpoint Node.js Agent
 * Copyright 2021-present NAVER Corp.
 * Apache License v2.0
 */

'use strict'

const log = require('../utils/logger')
const GrpcStream = require('./grpc-stream')

class GrpcBidirectionalStream {
    constructor(name, client, newStream) {
        this.grpcStream = new GrpcStream(name, client, newStream, () => {
            const stream = newStream.call(client)
            stream.on('data', (data) => {
                if (log.isDebug()) {
                    log.debug(`GrpcBidirectionalStream Server side stream data. ${name} on(data): ${data}`)
                }
            })
            stream.on('end', () => {
                if (log.isDebug()) {
                    log.debug(`GrpcBidirectionalStream Server side stream ended. ${name} on(end)`)
                }
    
                // https://www.grpc.io/docs/languages/node/basics/
                if (this.grpcStream.isLiveStream()) {
                    this.grpcStream.end()
                    log.debug(`GrpcBidirectionalStream Server side stream ended. But the client stream is active. ${this.name} on(end)`)
                }
            })
            stream.on('error', (e) => {
                if (log.isDebug() && e) {
                    log.debug(`GrpcBidirectionalStream Server side stream on error. ${name} on(error): ${JSON.stringify(e)}`)
                }
            })
            stream.on('status', (status) => {
                if (log.isDebug()) {
                    log.debug(`GrpcBidirectionalStream Server side stream data. ${name} on(status): ${JSON.stringify(status)}`)
                }
            })
            return stream
        })
    }

    write(data) {
        this.grpcStream.write(data)
    }

    end() {
        this.grpcStream.end()
    }
}

module.exports = GrpcBidirectionalStream
