...
class LogBuilder {
    static createGrpcLogBuilder() {
        return new LogBuilder('grpcLogger')
    }

    static buildGrpcLog() {
        return this.createGrpcLogBuilder().build()
    }
    ...
    build() {
        ...
        const logLevels = this.config.getLogLevels()
        const level = this.valueOfLoggingLevel(logLevels[this.name]) ?? this.level
        ...
        log.appenders = this.appenders.map(appender => {
            ...
            const specificLevel = this.valueOfLoggingLevel(logLevels[specificKey])
            const globalLevel = this.valueOfLoggingLevel(logLevels[globalKey])
    ...
    setConfig(config) {
        if (typeof config?.getLogLevels !== 'function') {
            return this
        }
