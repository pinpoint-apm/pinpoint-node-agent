...
test.skip('should render /app/og', async (t) => {
    try {
        const res = await next().fetch('/app/og')
        t.equal(res.status, 200, '/app/og responds with 200')
    } catch (err) {
        t.fail(err.message)
    }
    t.end()
...
test.skip('should render /app/og-node', async (t) => {
    try {
        const res = await next().fetch('/app/og-node')
        t.equal(res.status, 200, '/app/og-node responds with 200')
    } catch (err) {
        t.fail(err.message)
    }
    t.end()
