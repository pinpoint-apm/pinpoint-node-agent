...
const valueOfNumber = (envName, defaultValue) => {
  return () => {
    if (process.env[envName] === undefined) {
      return undefined
    }

    if (typeof defaultValue !== 'undefined' && Number.isInteger(defaultValue)) {
...
const ENV_MAP = {
  ...
  traceExclusionUrlCacheSize: valueOfNumber('PINPOINT_TRACE_EXCLUSION_URL_CACHE_SIZE', 100),
}
...
const configurationValueValidations = {
  validateTraceExclusionUrlCacheSize: () => {
    if (typeof agentConfig.traceExclusionUrlCacheSize !== 'undefined' && typeof agentConfig.traceExclusionUrlPatterns === 'undefined') {
      ...
      log.warn(`You have to set the trace-exclusion-url{ pattern: 'pattern', 'cache-size': 100} for using cache size.`)
    }
  }
}
