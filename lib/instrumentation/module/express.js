...
module.exports = function (agent, version, express) {
  ...
  InstrumentMethod.make(express.Router, 'use', traceContext).addScopedInterceptor(new ExpressUseInterceptor(traceContext))
  InstrumentMethod.make(express.Router, 'route', traceContext).addScopedInterceptor(new ExpressRouteInterceptor(traceContext))
