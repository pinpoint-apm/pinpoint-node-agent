...
const ENV_MAP = {
  ...
  unstableIdMaxLen: valueOfNumber('PINPOINT_UNSTABLE_ID_MAX_LEN'),
}
...
const configurationValueValidations = {
  ...
  validateIds: () => {
    [{ id: agentConfig.agentId, name: 'Agent ID' }, { id: agentConfig.applicationName, name: 'Application Name' }].filter(id => id.id)
      .filter(id => {
        const ID_MAX_LEN = agentConfig.unstableIdMaxLen || 24
        const idRegex = /[a-zA-Z0-9\\._\\-]+/

        if (agentConfig.unstableIdMaxLen) {
          log.warn(`The value .env "PINPOINT_UNSTABLE_ID_MAX_LEN" is not recommended.`)
        }
