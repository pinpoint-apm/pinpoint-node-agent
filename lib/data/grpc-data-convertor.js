...
const convertSpanChunk = (spanChunk) => {
  ...
  const pTransactionId = convertTransactionId(spanChunk.transactionId)
  const pSpanChunk = new spanMessages.PSpanChunk()
  pSpanChunk.setVersion(spanChunk.version);
...
const convertLocalAsyncId = (asyncId) => {
  if (!asyncId) {
    return null
  }

  const pLocalAsyncId = new spanMessages.PLocalAsyncId()
  pLocalAsyncId.setAsyncid(asyncId)
  pLocalAsyncId.setSequence(0)
