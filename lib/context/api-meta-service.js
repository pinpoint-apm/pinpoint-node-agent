...
class StringMetaCache {
  ...
  cacheAPIMeta(methodDescriptorBuilder) {
    ...
    const apiDescriptor = methodDescriptorBuilder.apiDescriptor
    const cachedValue = this.cache.get(apiDescriptor)

    if (cachedValue === null) {
      methodDescriptorBuilder.apiId = apiMetaCacheKeyGenerator.next
      this.cache.put(apiDescriptor, methodDescriptorBuilder.apiId)
