...
const { ConfigBuilder: ConfigBuilder2 } = require('./config-builder')
...
const valueValidations = {
  validateTraceExclusionUrlCacheSize: (config) => {
    if (typeof config.traceExclusionUrlCacheSize !== 'undefined' && typeof config.traceExclusionUrlPatterns === 'undefined') {
      ...
      log.warn(`You have to set the PINPOINT_TRACE_EXCLUSION_URL_PATTERN, PINPOINT_TRACE_EXCLUSION_URL_CACHE_SIZE or trace-exclusion-url{ pattern: 'pattern', 'cache-size': 100} for using exclusion url cache.`)
    }
...
const getConfig = (json) => {
  if (!configInstance) {
    configInstance = new ConfigBuilder2(json).build()
  }
