...
class GrpcDataSender {
  constructor(collectorIp, collectorTcpPort, collectorStatPort, collectorSpanPort, agentInfo, config) {
    ...
    this.commandEchoDeadlineOptionBuilder = new DeadlineOptionsBuilder()
  }
  ...
  sendApiMetaInfo(apiMetaInfo, callArguments) {
    ...
    if (log.isDebug()) {
      log.debug(`sendApiMetaInfo pApiMetaData: `, pApiMetaData.toObject())
    }
  ...
  sendStringMetaInfo(stringMetaInfo, callArguments) {
    ...
    if (log.isDebug()) {
      log.debug(`sendStringMetaInfo pStringMetaData: `, pStringMetaData.toObject())
    }
  ...
  sendSqlMetaInfo(sqlMetaData, callArguments) {
    ...
    if (log.isDebug()) {
      log.debug(`sendSqlMetaInfo sqlMetaData: `, pSqlMetaData.toObject())
    }
    callArguments = guardCallArguments(callArguments)
    let options = callArguments.getOptions()
    const callback = callArguments.getCallback()
  ...
  sendCommandEcho(commandEchoResponse, callback) {
    let options = this.commandEchoDeadlineOptionBuilder.build().getOptions()
  ...
  sendPing() {
    ...
    if (log.isDebug()) {
      log.debug(`sendPing pPing: `, pPing.toObject())
    }
...
function isUnknownError(err) {
  return err && !isCancelledError(err)
}
