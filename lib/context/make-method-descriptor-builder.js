...
const makeMethodDescriptorBuilder = (moduleName, index) => {
    ...
    captureNamedGroup(stringStackOfCallStack(callstack, index))
...
const captureNamedGroup = (stack) => {
    const functionNameMatches = [typeFunctionNameMethodName, newFunctionName]
...
const newFunctionName = (stack) => {
    ...
    const functionName = Array.from(group.groups.functionName)
        .map((element, index) => {
