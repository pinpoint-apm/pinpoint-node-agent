  ...
  shimmer.wrap(express.Router, 'use', function (original) {
    return function () {
      ...
      if (fn && fn.name !== 'router' && this.stack && this.stack.length) {
        ...
        patchLayer(layer, 'middleware', layer.name, fn.length)
      }
      return result
    }
  })
  ...
  shimmer.wrap(express.Router, 'route', function (original) {
    return function route() {
      ...
      if (this.stack && this.stack.length) {
        const layer = this.stack[this.stack.length - 1]
        patchLayer(layer, 'route', layer.name)
      }
      return route
    }
  })

  function patchLayer(layer, objectName, layerName, handlerArgumentsLength) {
