...
class TraceBuilder {
    ...
    buildAsyncTrace(callerSpanEventRecorder) {
        if (!callerSpanEventRecorder || typeof callerSpanEventRecorder.getLocalAsyncId !== 'function' || typeof callerSpanEventRecorder.recordNextAsyncId !== 'function') {
            return new DisableAsyncTrace()
        }
        const asyncId = callerSpanEventRecorder.getLocalAsyncId()
        
        if (typeof asyncId.getAsyncId !== 'function' || typeof asyncId.getSequence !== 'function') {
            return new DisableAsyncTrace()
        }
        callerSpanEventRecorder.recordNextAsyncId(asyncId.getAsyncId())
