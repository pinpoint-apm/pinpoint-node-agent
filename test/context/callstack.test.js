...
test(`span and spanEvent call stack`, async (t) => {
    ...
    axios.get(`https://eonet.sci.gsfc.nasa.gov/api/v2.1/categories`)
        .then(function (response) {
            ...
            t.equal(agent.traceContext.currentTraceObject(), trace, "current trace is current asyncId trace object")
            t.equal(agent.pinpointClient.dataSender.mockSpanChunk.spanEventList.length, 2, "spanEventList length")
            t.equal(agent.pinpointClient.dataSender.mockSpanChunk.spanEventList[1].annotations[0].key, 12, "APIDesc key")
            t.equal(agent.pinpointClient.dataSender.mockSpanChunk.spanEventList[1].annotations[0].value.stringValue, "GET", "APIDesc stringValue")
            t.equal(agent.pinpointClient.dataSender.mockSpanChunk.spanEventList[1].annotations[1].key, 40, "HTTP.URL key")
            t.equal(agent.pinpointClient.dataSender.mockSpanChunk.spanEventList[1].annotations[1].value.stringValue, 'eonet.sci.gsfc.nasa.gov/api/v2.1/categories', "HTTP.URL stringValue")
            t.equal(agent.pinpointClient.dataSender.mockSpanChunk.spanEventList[1].annotations[2].key, 46, "HTTP.status.code")
            t.equal(agent.pinpointClient.dataSender.mockSpanChunk.spanEventList[1].annotations[2].value.intValue, 200, "HTTP.status.code stringValue")
