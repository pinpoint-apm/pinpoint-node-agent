...
class SpanEventRecorder {
    static nullObject(traceRoot, sqlMetadataService) {
        return new SpanEventRecorder(SpanEventBuilder.nullObject(), traceRoot, sqlMetadataService)
    }
    ...
    recordSqlInfo(sql, bindString) {
        ...
        const parsingResult = this.sqlMetadataService.cacheSql(sql)
