...
test(`${testName1} Should record request in basic route`, function (t) {
  ...
  app.get(PATH, async (req, res) => {
    ...
    agent.callbackTraceClose((trace) => {
      ...
      t.equal(trace.spanBuilder.annotations[1].key, annotationKey.HTTP_URL.code, 'HTTP URL key match')
  ...
  app.post(PATH, (req, res) => {
    ...
    agent.callbackTraceClose((trace) => {
      t.equal(trace.spanBuilder.annotations[0].key, annotationKey.HTTP_URL.code, 'HTTP URL key match')
...
test('incoming request by Disable Trace requests', (t) => {
  ...
  const server = app.listen(TEST_ENV.port, async function () {
    ...
    t.equal(actualTrace.spanBuilder.annotations[0].key, annotationKey.HTTP_URL.code, 'HTTP URL key match')
    ...
    t.equal(actualApiTrace.spanBuilder.annotations[0].key, annotationKey.HTTP_URL.code, 'HTTP URL key match')
