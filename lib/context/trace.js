...
class Trace {
  ...
  startSpanEvent(builder) {
    const now = Date.now()
    const spanEvent = new SpanEvent.builder(this.span, this.sequence)
      .setCallStackLength(this.callStack.length)
      .setStartTime(now)
      .setStartElapsed(now - this.span.startTime)
      .build()
    ...
    this.spanEventRecorder = new SpanEventRecorder(spanEvent, this.span)
    this.spanEventRecorder.record(builder)
    return this.spanEventRecorder
  }
