...
class Agent {
    constructor(agentInfo, dataSender, config, logger) {
        this.agentInfo = agentInfo
        this.dataSender = dataSender
        this.config = config
        this.logger = logger

        this.traceContext = new TraceContext(agentInfo, dataSender, config)
    }
...
class AgentBuilder {
    ...
    addTraceCompletionHook(hook) {
        if (typeof hook?.onComplete !== 'function') {
            return this
        }

        this.traceCompletionHooks.push(hook)
        return this
    }
    ...
    build() {
        ...
        const agent = new Agent(this.agentInfo, this.dataSender, this.config, this.logger ?? logger.getLogger())
        agent.traceContext.traceCompletionHooks = this.traceCompletionHooks
